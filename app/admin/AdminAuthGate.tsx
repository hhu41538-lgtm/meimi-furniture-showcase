      ...
      <section className="auth-gate-panel" aria-labelledby="auth-title">
        ...
        <form className="auth-gate-form" onSubmit={submit}>
          ...
          <label>
            ...
            <span className="auth-gate-input-wrap"><KeyRound size={16} /><input type={showLoginKey ? "text" : "password"} inputMode={mode === "admin" ? "numeric" : "text"} minLength={mode === "sales" && salesAction === "register" ? 6 : undefined} maxLength={mode === "sales" && salesAction === "register" ? 32 : undefined} pattern={mode === "sales" && salesAction === "register" ? "\\S{6,32}" : undefined} value={loginKey} onChange={(event) => setLoginKey(event.target.value)} placeholder={mode === "admin" ? "输入管理员密钥" : "输入销售密钥"} autoComplete="current-password" autoCapitalize="none" autoCorrect="off" spellCheck={false} autoFocus={mode === "admin" || salesAction === "login"} /><button className="auth-gate-password-toggle" type="button" onClick={() => setShowLoginKey((current) => !current)} aria-label={showLoginKey ? "隐藏密钥" : "显示密钥"} aria-pressed={showLoginKey} title={showLoginKey ? "隐藏密钥" : "显示密钥"}>{showLoginKey ? <EyeOff size={16} /> : <Eye size={16} />}</button></span>
          </label>
          {mode === "sales" && salesAction === "register" ? (
            <label>确认销售密钥<span className="auth-gate-input-wrap"><input type={showConfirmKey ? "text" : "password"} minLength={6} maxLength={32} value={confirmKey} onChange={(event) => setConfirmKey(event.target.value)} placeholder="再次输入销售密钥" autoComplete="new-password" autoCapitalize="none" autoCorrect="off" spellCheck={false} /><button className="auth-gate-password-toggle" type="button" onClick={() => setShowConfirmKey((current) => !current)} aria-label={showConfirmKey ? "隐藏确认密钥" : "显示确认密钥"} aria-pressed={showConfirmKey} title={showConfirmKey ? "隐藏确认密钥" : "显示确认密钥"}>{showConfirmKey ? <EyeOff size={16} /> : <Eye size={16} />}</button></span></label>
          ) : null}
